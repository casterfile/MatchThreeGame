...
public static class GameUtilities
{
    ...
    public static IEnumerator AnimatePotentialMatches(IEnumerable<GameObject> potentialMatches)
    {
        for (float i = 1f; i >= 0.3f; i -= 0.1f)
        {
            ...
            yield return new WaitForSeconds(ConstantsVariable.OpacityAnimationFrameDelay);
        }
    ...
    public static IEnumerable<GameObject> GetPotentialMatches(ShapesArray shapes)
    {
        ...
        for (int row = 0; row < ConstantsVariable.Rows; row++)
        {
            for (int column = 0; column < ConstantsVariable.Columns; column++)
            {
                ...
                if(row >= ConstantsVariable.Rows / 2 && matches.Count > 0 && matches.Count <=2)
                    return matches[UnityEngine.Random.Range(0, matches.Count - 1)];
            }
        }
        return null;
    ...
    {
        if (column <= ConstantsVariable.Columns - 2)
        {
                ...
                IsSameType(shapes[row, column + 1].GetComponent<Shape>()))
            {
                ...
                if (row <= ConstantsVariable.Rows - 2 && column >= 1)
                    if (shapes[row, column].GetComponent<Shape>().
                    IsSameType(shapes[row + 1, column - 1].GetComponent<Shape>()))
                        return new List<GameObject>()
                                {
    ...
    {
        if (column <= ConstantsVariable.Columns - 3)
        {
            if (shapes[row, column].GetComponent<Shape>().
                IsSameType(shapes[row, column + 1].GetComponent<Shape>()))
            {

                if (row >= 1 && column <= ConstantsVariable.Columns - 3)
                    if (shapes[row, column].GetComponent<Shape>().
                ...
                if (row <= ConstantsVariable.Rows - 2 && column <= ConstantsVariable.Columns - 3)
                    if (shapes[row, column].GetComponent<Shape>().
                    IsSameType(shapes[row + 1, column + 2].GetComponent<Shape>()))
                        return new List<GameObject>()
                                {
    ...
    {
        if (row <= ConstantsVariable.Rows - 3)
        {
            if (shapes[row, column].GetComponent<Shape>().
                IsSameType(shapes[row + 1, column].GetComponent<Shape>()))
            {
