...
public class GameControllerManager : MonoBehaviour
{
    ...
    public void InitializeCandyAndSpawnPositions()
    {
        ...
        SpawnPositions = new Vector2[ConstantsVariable.Columns];
    ...
    private void SetupSpawnPositions()
    {
        ...
        for (int column = 0; column < ConstantsVariable.Columns; column++)
        {
            SpawnPositions[column] = BottomRight
                + new Vector2(column * CandySize.x, ConstantsVariable.Rows * CandySize.y);
        }
    }
    ...
    private IEnumerator FindMatchesAndCollapse(RaycastHit2D hit2)
    {
        ...
        hitGo.transform.positionTo(ConstantsVariable.AnimationDuration, hitGo2.transform.position);
        hitGo2.transform.positionTo(ConstantsVariable.AnimationDuration, hitGo.transform.position);
        yield return new WaitForSeconds(ConstantsVariable.AnimationDuration);
        ...
        if (totalMatches.Count() < ConstantsVariable.MinimumMatches)
        {
            hitGo.transform.positionTo(ConstantsVariable.AnimationDuration, hitGo2.transform.position);
            hitGo2.transform.positionTo(ConstantsVariable.AnimationDuration, hitGo.transform.position);
            yield return new WaitForSeconds(ConstantsVariable.AnimationDuration);
        ...
        bool addBonus = totalMatches.Count() >= ConstantsVariable.MinimumMatchesForBonus &&
            !BonusTypeUtilities.ContainsDestroyWholeRowColumn(hitGoMatchesSet.BonusesContained) &&
            !BonusTypeUtilities.ContainsDestroyWholeRowColumn(hitGo2MatchesSet.BonusesContained);
        ...
        int timesRun = 1;
        while (totalMatches.Count() >= ConstantsVariable.MinimumMatches)
        {
            //increase score
            IncreaseScore((totalMatches.Count() - 2) * ConstantsVariable.Match3Score);

            if (timesRun >= 2)
                IncreaseScore(ConstantsVariable.SubsequentMatchScore);
            ...
            yield return new WaitForSeconds(ConstantsVariable.MoveAnimationMinDuration * maxDistance);
    ...
    private void MoveAndAnimate(IEnumerable<GameObject> movedGameObjects, int distance)
    {
        ...
        {
            item.transform.positionTo(ConstantsVariable.MoveAnimationMinDuration * distance, BottomRight +
    ...
    private void RemoveFromScene(GameObject item)
    {
        ...
        Destroy(newExplosion, ConstantsVariable.ExplosionDuration);
        Destroy(item);
    }
